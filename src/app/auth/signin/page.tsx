...
import { PrivacyConsentModal } from '@/shared/ui/PrivacyConsentModal';
...
function SignInContent({
  ...
  const startOAuth = (provider: 'google' | 'yandex' | 'github') => {
    setError('');
  ...
  const handleConsentAccept = async () => {
    ...
    setOauthLoading(pendingOAuth);

    try {
      window.location.href = `/api/auth/oauth/${pendingOAuth}?callbackUrl=${encodeURIComponent(callbackUrl)}`;
    } catch {
